...
static GLfloat sizePt     = 8.75f; // taille des points
...
static void makeImage(grid *G) {
  ...
  if (dmax == 0)
    dmax = 1E-10; // pour éviter la division par 0
...
bool NextPermutation(int *P, int n) {
  ...
  */
  ...
  for (j = 0; j < m; j++)
    if (P[j] < P[j + 1])
      i = j; /* on a trouvé un i tq P[i]<P[i+1] */
  if (i < 0)
      ...
      if (value[x0 * y + y0] == -1) {
        ...
        if (value[x0 * y + y0] > 0) {
          ...
          do {
            ...
            if (x_0 < x_1)
              for (int i = 0; i < w; ++i)
                Gw.value[x_1 * (w + 1)][y_0 * (w + 1) + i + 1] = V_FREE;
            if (x_0 > x_1)
              for (int i = 0; i < w; ++i)
                Gw.value[x_0 * (w + 1)][y_0 * (w + 1) + i + 1] = V_FREE;
            if (y_0 < y_1)
              for (int i = 0; i < w; ++i)
                Gw.value[x_1 * (w + 1) + i + 1][y_1 * (w + 1)] = V_FREE;
            if (y_0 > y_1)
              for (int i = 0; i < w; ++i)
                Gw.value[x_1 * (w + 1) + i + 1][y_0 * (w + 1)] = V_FREE;
            i0        = i1;
            i1        = value[i0] - 1;
            value[i0] = 0;
            count++;
  ...
  for (int t = 0; t < m; t++) // répète m fois
    for (int i = 1; i < G.X - 1; i++)
      for (int j = 1; j < G.Y - 1; j++) {
        ...
        for (int k = 0; k < 8; k++)
          if (G.value[i + V[k][0]][j + V[k][1]] == type)
            c++;
        if (c && random() % ((8 - c) * 20 + 1) == 0)
          G.value[i][j] = type;
      }
}
...
void addRandomArc(grid G, int type, int n) {
  ...
  for (int i = 0; i < n; i++) { // on répète n fois
    ...
    if (a1 < a2)
      SWAP(a1, a2, t); // ici a1>a2
    if (a1 - a2 > M_PI / 3)
      a2 = (a1 + a2) / 2; // si arc trop grand
    ...
    for (int j = 0; j < (int)(1.5 * d); j++) {
      ...
      if (!inGrid(&G, u) || onBorder(&G, u.x, u.y))
        continue;
...
static void centering(double margin) {
  point Pmin = POINT[0];
  point Pmax = Pmin;
  ...
  if (s == 0)
    s = m; // si tous les points sont confondus
...
static int fcmp_angle(const void *P, const void *Q) {
  double const p = angle(((const point *)P)->x, ((const point *)P)->y);
  double const q = angle(((const point *)Q)->x, ((const point *)Q)->y);
  return (p > q) - (p < q);
}
...
GLfloat COLOR_GROUND[] = {1.0, 1.0, 1.0}; // fond
GLfloat COLOR_POINT[]  = {1.0, 0.0, 0.0}; // point
GLfloat COLOR_LINE[]   = {0.0, 0.0, 0.0}; // ligne de la tournée
...
static void drawX(point *V, int n, int *P, int k, graph *G, bool back) {
  ...
  if (quadrillage) {
    ...
    double e = m / (thek - 1);        // écart entre les croix
    point z  = (point){(width - e * (thek - 1)) / 2,
                       (height - e * (thek - 1)) / 2}; // point zéro
    // on place les thek croix sur la diagonale allant de z+(0,0) à z+(m,m)
    for (int i = 0; i < thek; i++) { // thek croix, dont une au centre
...
static void waitGridDelay(grid G, unsigned int delay,
